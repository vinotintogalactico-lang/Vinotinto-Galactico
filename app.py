...
div[data-testid="stButton"] button { font-family: 'Bebas Neue', sans-serif !important; font-size: 1.2rem !important; letter-spacing: 3px !important; border: none !important; border-radius: 6px !important; width: 100% !important; padding: .7rem 1rem !important; color: white !important; }
...
.news-card-title { font-family: 'Bebas Neue', sans-serif; font-size: 1.4rem; }
...
try: banner_html = f'<img src="data:image/png;base64,{_b64("banner-vinotinto.png")}" alt="banner">'
except: banner_html = ""
try: logo_src = f"data:image/jpeg;base64,{_b64('Logo.jpg')}"
except: logo_src = ""

if "modo" not in st.session_state: st.session_state.modo = "vg"
if "resultado" not in st.session_state: st.session_state.resultado = None

if banner_html: st.markdown(f'<div class="vg-banner-wrap">{banner_html}</div>', unsafe_allow_html=True)
...
with st.sidebar:
    ...
    for cat, fuentes in sources.items():
        st.markdown(f'<div class="cat-label cat-vg">{CATEGORY_ICONS.get(cat, "📌")} {cat}</div>', unsafe_allow_html=True)
        for f in fuentes:
...
with col_title:
    st.markdown(f"<h2 style='font-family:Bebas Neue;letter-spacing:2px;'>{emoji_modo} NEWS EXTRACTOR · {modo.upper()}</h2>", unsafe_allow_html=True)

with col_prensa:
    st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
    path_p = Path("Prensa Deportiva/Prensa_Deportiva.html")
    if path_p.exists():
        b64_p = base64.b64encode(path_p.read_bytes()).decode()
        st.markdown(f'<a href="data:text/html;base64,{b64_p}" target="_blank" style="text-decoration:none;"><div style="background:linear-gradient(135deg,#7a1a2e,#c0392b);color:white;padding:10px;border-radius:6px;text-align:center;font-family:Bebas Neue;cursor:pointer;">🗞️ PRENSA DEPORTIVA</div></a>', unsafe_allow_html=True)
...
if st.session_state.resultado:
    ...
    for n in res["noticias"]:
        st.markdown(f'<div class="news-card"><div class="news-card-title"><a href="{n["url"]}" target="_blank">{n["title"]}</a></div><div class="news-body">{n["body"][:500]}...</div></div>', unsafe_allow_html=True)