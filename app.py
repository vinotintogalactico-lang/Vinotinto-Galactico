...
# Ruta absoluta al archivo Prensa_Deportiva.html
PRENSA_DEPORTIVA_ABSOLUTE = str(Path("Prensa Deportiva/Prensa_Deportiva.html").resolve())
...
/* Botón EXTRAER NOTICIAS */
div[data-testid="stButton"] button[key="btn_extraer"]{{
    background:{accent} !important;
    border:none !important;
    box-shadow:0 4px 15px {'rgba(192,57,43,.4)' if modo=='vg' else 'rgba(46,125,50,.4)'} !important;
}}
...
if modo == "vg":
    ...
    CATEGORY_ICONS = {}  # Las categorías ya tienen emoji en el nombre
    cat_css = "cat-mun"
...
with col_prensa:
    ...
    if modo == "vg":
        if Path(PRENSA_DEPORTIVA_ABSOLUTE).exists():
            btn_color = "#7a1a2e"
            btn_shadow = "rgba(192,57,43,.4)"
            btn_prensa = f"""<style>
body{{margin:0;padding:0;background:transparent;overflow:hidden;}}
a{{
  display:block;
  background:linear-gradient(135deg,{btn_color},#c0392b);
  ...
  width:100%;box-shadow:0 4px 15px {btn_shadow};
  transition:all .2s;box-sizing:border-box;
  text-align:center;text-decoration:none;
}}
a:hover{{transform:translateY(-1px);filter:brightness(1.15);}}
</style>
<a href="file:///{PRENSA_DEPORTIVA_ABSOLUTE.replace('\\','/')}" target="_blank">🗞️ PRENSA DEPORTIVA</a>"""
