...
class EspnDeportesExtractor(GenericExtractor):
    ...
    async def _get_article_links(self, page: Page) -> list[str]:
        bad = ["/video/", "/scores/", "/standings/", "/schedule/",
               "/resultados/", "/calendario/", "/clasificacion/"]
        selectors = [
            ".contentItem a[href]",
            ".headlineStack__list a[href]",
            ".story a[href]",
            "h1 a[href]",
            "h2 a[href]",
            "h3 a[href]",
    ...
    async def _extract_article(self, context, url: str) -> dict | None:
        ...
        try:
            ...
            subtitle = soup.select_one(".article-body__subtitle, .subhead")
            subtitle_text = subtitle.get_text(strip=True) if subtitle else ""

            author = soup.select_one(".author-name, .byline__author-name")
            ...
            if time_el and time_el.get("datetime"):
                ...
                d = soup.select_one("time, .timestamp")
