...
class TycSportsExtractor(GenericExtractor):
    ...
    async def _extract_article(self, context, url: str) -> dict | None:
        ...
        try:
            from mundial.mundial_core.article_parser import parse_article
            ...
            subtitle = soup.select_one(".article__subtitle, .bajada, h2")
            subtitle_text = subtitle.get_text(strip=True) if subtitle else ""

            author = soup.select_one(".article__author, .author-name, .autor")
            ...
            if time_el and time_el.get("datetime"):
                ...
                d = soup.select_one("time, .article__date, .date")
